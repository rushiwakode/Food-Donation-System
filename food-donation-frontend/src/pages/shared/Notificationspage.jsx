...
const TYPE_META = {
  ...
  SYSTEM:   { icon: InformationCircleIcon,      color: 'text-slate-500',   bg: 'bg-slate-100 dark:bg-slate-800',       label: 'System'   },
  INFO:     { icon: InformationCircleIcon,      color: 'text-blue-500',    bg: 'bg-blue-50 dark:bg-blue-900/20',       label: 'Info'     },
};

const FILTER_TABS = ['ALL', 'UNREAD', 'DONATION', 'CLAIM', 'DELIVERY', 'ALERT'];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
            ...
            {unreadCount > 0 && (
              <span className="ml-2 badge badge-info">{unreadCount} unread</span>
            )}
          </p>
        </div>
        {unreadCount > 0 && (
      ...
      {loading ? (
        ...
        <div className="card p-16 text-center">
          <div className="w-16 h-16 rounded-2xl bg-primary-50 dark:bg-primary-900/20 flex items-center justify-center mx-auto mb-4">
            <BellIcon className="w-8 h-8 text-primary-300 dark:text-primary-600" />
          </div>
          ...
          <p className="text-slate-400 dark:text-slate-500 text-sm">
            {activeFilter === 'UNREAD'
              ...
              : 'Notifications about donations, claims, and deliveries will appear here.'}
          </p>
        </div>
      ) : (
        <AnimatePresence>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {!n.isRead && <span className="w-2 h-2 rounded-full bg-primary-500 animate-pulse-slow" />}
